...
namespace PrimeraAplicacio
{
    ...
    public class ExcelCrearPlanos : IExternalCommand
    {
          ...
          ElementSet elements)
        {
            ...
            try
            {
                ...
                //Conseguimos el titleblock que queremos:
                ElementId bloqueTituloId = new FilteredElementCollector(doc).OfCategory(BuiltInCategory.OST_TitleBlocks).WhereElementIsElementType().First(x => x.Name == "A1 metric").Id;
                ...
                using(Transaction tran = new Transaction(doc,"Creación planos"))
                {
                    ...
                    {
                        ViewSheet plano = ViewSheet.Create(doc, bloqueTituloId);
                        //Asignamos valores a los parametros de los planos:
                        plano.SheetNumber = lF[0];
                        plano.Name = lF[1];
                    }
                    tran.Commit();
                }
